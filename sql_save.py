...
def model_metrics(model_name, accuracy, model_precision, recall, f1):
    conn = mysql.connector.connect(
        host="localhost",
        user="root",
        password="magda2000",
    ...
    sql = """INSERT INTO model_metrics (model_name, accuracy, model_precision, recall, f1_score)
             VALUES (%s, %s, %s, %s, %s)"""

    cursor.execute(sql, (model_name, accuracy, model_precision, recall, f1))
    conn.commit()
    conn.close()
