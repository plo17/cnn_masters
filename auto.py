...
def build_model(hp):
    model = keras.Sequential([
        ...
        data_augmentation,
        # liczbą filtrów dobierana przez tuner
        keras.layers.Conv2D(hp.Choice('filters', [32, 64, 128]), (3, 3), activation='relu'),
        keras.layers.MaxPooling2D(2, 2),

        keras.layers.Flatten(),

        # Warstwa gęsta z neuronami wybieranymi przez tuner
...
print(f"Najlepsze hiperparametry: filters={best_hps.get('filters')}, dense_units={best_hps.get('dense_units')}, learning_rate={best_hps.get('learning_rate')}")
